export const BodyParamsMetadataKey = 'route:body';

export const QueryParamsMetadataKey = 'route:params';

export const ModuleMetadataKey = 'module:metadata';

export const INJECTION_TOKEN_MK = 'injection:token';

export const INJECT_MK = 'inject:';

export const ControllerMetadataKey = 'controller:metadata';

export const CoremoduleMetadataKey = 'coremodule:metadata';

export const GuardsMetadataKey = 'guards';
